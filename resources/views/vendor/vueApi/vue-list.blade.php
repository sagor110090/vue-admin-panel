            ...
            <div class="col-12" v-if="newRow == false">
                <div class="table-responsive p-3">
                    ...
                    <table class="table table-bordered" id="table" v-if="{{ $data['plural_lower'] }}.data.length > 0">
                        ...
                        <tbody>
                            <tr v-for="({{ $data['singular_lower'] }},index) in {{ $data['plural_lower'] }}.data" :key="{{ $data['singular_lower'] }}.id">
                                @foreach($data['fields'] as $field)
                                @if ($field['name'] == 'id')
                                <td> @{{ index+1 }} </td>
                ...
                <span v-else-if="!{{ $data['plural_lower'] }}.data">Loading...</span>
                <span v-else>No {{ $data['plural_lower'] }} exist</span>
                <pagination
                :data="{{ $data['plural_lower'] }}"
                @pagination-change-page="list{{ $data['plural'] }}"
              ></pagination>
            </div><!-- End 2nd half -->
            </div>
        </div>
...
import { Form, HasError, AlertError } from 'vform'
    export default {
        ...
        methods: {
            ...
            var that = this;
            this.form.get('{{config('vueApi.vue_url_prefix')}}/{{ $data['plural_lower'] }}?page='+ page).then(function(response){
                that.{{ $data['plural_lower'] }} = response.data;
            })
            ...
            if (willDelete) {
                this.form.delete('{{config('vueApi.vue_url_prefix')}}/{{ $data['plural_lower'] }}/'+{{ $data['singular_lower'] }}.id).then(function(response){
                    that.{{ $data['plural_lower'] }}.data.splice(index,1);
            });
            }
